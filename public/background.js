...
function updateTimes() {
  if (activeDomain && startTime) {
    ...
    console.log('Updated domainTimes:', domainTimes); 
    console.log('Updated dailyGoals:', dailyGoals); 
    chrome.storage.local.set({ domainTimes, dailyGoals }); s
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === 'setGoals') { 
        dailyGoals = message.goals; 
        chrome.storage.local.set({ dailyGoals }); 
    }
